...
class Single_Image_Widget extends WP_Widget {
	...
	public function __construct( $id_base = false, $name = false, $widget_options = array(), $control_options = array() ) {
		$id_base = ( $id_base ) ? $id_base : 'singleimage'; // Legacy ID.
		$name    = ( $name ) ? $name : __( 'Image', 'single-image-widget' );

		$widget_options = wp_parse_args(
			$widget_options,
			array(
				'classname'                   => 'widget_singleimage', // Legacy class name.
				'customize_selective_refresh' => true,
				'description'                 => __( 'An image from your Media Library.', 'single-image-widget' ),
			)
		);
	...
	public function widget( $args, $instance ) {
		$cache    = (array) wp_cache_get( 'Single_Image_Widget', 'widget' );
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'Single_Image_Widget', array_filter( $cache ), 'widget' );
		}
	}
	...
	public function render( $args, $instance ) {
		 ...
		 */
		$inside = apply_filters( 'Single_Image_Widget_output', '', $args, $instance, $this->id_base );
		...
		if ( $inside ) {
			...
			$data['image_size'] = $image_size = ( ! empty( $instance['image_size'] ) ) ? $instance['image_size'] : apply_filters( 'Single_Image_Widget_output_default_size', 'medium', $this->id_base );
			$data['title'] = ( empty( $instance['title'] ) ) ? '' : $instance['title'];
			$data = array_merge( $instance, $data );
			$data = apply_filters( 'Single_Image_Widget_template_data', $data );

			ob_start();
			$templates = $this->get_template_names( $args, $instance );

			$template_loader = new Single_Image_Widget_Template_Loader();
	...
	public function form( $instance ) {
		...
		$button_class = array( 'button', 'button-hero', 'single-image-widget-control-choose' );
		 ...
		 * @param string $id_base Widget type id.
		 */
		$fields = (array) apply_filters( 'Single_Image_Widget_fields', $this->form_fields(), $this->id_base );
		?>

		<div class="single-image-widget-form">
			 ...
			 * @param string $id_base  Widget type id.
			 */
			do_action( 'Single_Image_Widget_form_before', $instance, $this->id_base );
			?>

			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php _e( 'Title:', 'single-image-widget' ); ?></label>
			...
			<?php if ( ! is_Single_Image_Widget_legacy() ) : ?>
				<p class="single-image-widget-control<?php echo ( $image_id ) ? ' has-image' : ''; ?>"
					data-title="<?php esc_attr_e( 'Choose an Image', 'single-image-widget' ); ?>"
					data-update-text="<?php esc_attr_e( 'Update Image', 'single-image-widget' ); ?>"
					...
					?>
					<input type="hidden" name="<?php echo esc_attr( $this->get_field_name( 'image_id' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'image_id' ) ); ?>" value="<?php echo absint( $image_id ); ?>" class="image-id single-image-widget-control-target">
					<a href="#" class="<?php echo esc_attr( join( ' ', $button_class ) ); ?>"><?php _e( 'Choose an Image', 'single-image-widget' ); ?></a>
				</p>
			<?php endif; ?>
					...
					switch ( $field ) {
						case 'image_size' :
							...
							<p class="<?php echo esc_attr( $this->siw_field_class( 'image_size' ) ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( 'image_size' ) ); ?>"><?php _e( 'Size:', 'single-image-widget' ); ?></label>
						...
						case 'link' :
							...
							<p class="<?php echo esc_attr( $this->siw_field_class( 'link' ) ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( 'link' ) ); ?>"><?php _e( 'Link:', 'single-image-widget' ); ?></label>
								<span class="single-image-widget-input-group">
									<input type="text" name="<?php echo esc_attr( $this->get_field_name( 'link' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'link' ) ); ?>" value="<?php echo esc_url( $instance['link'] ); ?>" class="single-image-widget-input-group-field">
									<button class="single-image-widget-find-posts-button single-image-widget-input-group-button dashicons dashicons-search"></button>
								</span>
							</p>
							<p class="<?php echo esc_attr( $this->siw_field_class( 'new_window' ) ); ?>" style="margin-top: -0.75em; padding-left: 2px">
								<label for="<?php echo esc_attr( $this->get_field_id( 'new_window' ) ); ?>">
									...
									<?php _e( 'Open in new window?', 'single-image-widget' ); ?>
								</label>
							</p>
							<?php
							break;
						...
						case 'link_classes' :
							...
							<p class="<?php echo esc_attr( $this->siw_field_class( 'link_classes' ) ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( 'link_classes' ) ); ?>"><?php _e( 'Link Classes:', 'single-image-widget' ); ?></label>
						...
						case 'link_text' :
							...
							<p class="<?php echo esc_attr( $this->siw_field_class( 'link_text' ) ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( 'link_text' ) ); ?>"><?php _e( 'Link Text:', 'single-image-widget' ); ?></label>
						...
						case 'text' :
							...
							<p class="<?php echo esc_attr( $this->siw_field_class( 'text' ) ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( 'text' ) ); ?>"><?php _e( 'Text:', 'single-image-widget' ); ?></label>
						...
						default :
							 ...
							 * registered with the 'Single_Image_Widget_fields'
							 * filter.
							 *
							 * @since 3.0.0
							 *
							 ...
							 */
							do_action( 'Single_Image_Widget_field-' . sanitize_key( $field ), $instance, $this );
					}
				}
			}
			 ...
			 * @param string $id_base  Widget type id.
			 */
			do_action( 'Single_Image_Widget_form_after', $instance, $this->id_base );
			?>

		</div><!-- /.single-image-widget-form -->
		<?php
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance = apply_filters( 'Single_Image_Widget_instance', $instance, $new_instance, $old_instance, $this->id_base );
	...
	public function get_image_sizes( $image_id ) {
		$sizes = array( 'full' => __( 'Full Size', 'single-image-widget' ) );

		$imagedata = wp_get_attachment_metadata( $image_id );
		if ( isset( $imagedata['sizes'] ) ) {
			$size_names = Single_Image_Widget_Plugin::get_image_size_names();
	...
	public function flush_group_cache( $post_id = null ) {
		...
		wp_cache_delete( 'Single_Image_Widget', 'widget' );
	}
	...
	protected function siw_field_class( $id ) {
		$classes = array( 'single-image-widget-field', 'single-image-widget-field-' . sanitize_html_class( $id ) );

		$hidden_fields = Single_Image_Widget_Plugin::get_hidden_fields();
